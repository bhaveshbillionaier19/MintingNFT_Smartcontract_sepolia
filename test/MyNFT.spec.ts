  ...
  describe("mintNft", async () => {
    it("emits the Transfer event", async () => {
      await expect(mintNftDefault())
        ...
        .withArgs(constants.AddressZero, wallet.address, "1");
    });

    it("returns the new item ID", async () => {
      await expect(
        await deployedContract.callStatic.awardItem(wallet.address, TOKEN_URI)
      ).to.eq("1");
    });
    ...
    it("cannot mint to address zero", async () => {
      const TX = deployedContract.awardItem(
        constants.AddressZero,
        TOKEN_URI
      );
